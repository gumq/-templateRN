...
const ChooseCompanyScreen = ({ route }) => {
  ...
  const onSubmit = async () => {
    const param = {
      ...
      CollectFromServer: body?.CollectFromServer,
      AppCode: "NLT_SC",
      CmpnID: valueCmpnID?.CmpnID?.toString(),
    };
    try {
      setSubmitting(true);
            ...
            <View style={styles.inputContainer}>
              <View style={styles.bodyInput}>
                <CardModalCompany
                  ...
                  value={valueCmpnID?.CompanyName}
                />
                <Button
                  disabled={valueCmpnID?.CompanyName ? false : true}
                  onPress={onSubmit}
